...
TWEET_STRATEGY = """
## バズるAI投稿の戦略（2026年版・ビジネス層向け）
1. 冒頭1行でスクロールを止める：数字・驚き・逆説で始める
   例）「Anthropicが初の黒字。売上$10.9B→利益$559M。」
   例）「AIが"答える"から"行動する"時代へ。」
2. 「知らないとヤバい」感を出す：静かな危機感・緊迫感
3. ビジネスパーソンが「自分事」として読める構造
   - 業界・職種への影響を具体的に示す
4. 問いで終わると引用RT・コメントが増える
   例）「日本企業はここに気づいているか？」
5. 短文→改行→核心→問い の構造が最も拡散されやすい
6. ハッシュタグは #AI #生成AI のうち1〜2個まで（文中ではなく末尾）
7. Noteリンクがある場合は文末に自然に追加
"""
